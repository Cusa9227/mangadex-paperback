...
exports.MangaFireInfo = {
    ...
    description: 'MangaFire source for Paperback',
...
class MangaFire extends types_1.Source {
    ...
    async getMangaDetails(mangaId) {
        ...
        const author = $('.meta span:contains("Author")')
            .next()
            .text()
            .trim();
        const statusText = $('.meta span:contains("Status")')
            .next()
            .text()
            .trim();
    ...
    async getChapterDetails(mangaId, chapterId) {
        const $ = await this.fetchCheerio(`${this.baseUrl}/read/${mangaId}/${chapterId}`);
        const pages = [];
    ...
    async getSearchResults(query, _metadata) {
        ...
        const $ = await this.fetchCheerio(`${this.baseUrl}/filter?keyword=${search}`);
        const results = [];
        $('.item').each((_, el) => {
